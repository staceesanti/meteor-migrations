...
function unexpandedMigrations () {
  if (!Migrations.generateServerId) {
    console.log("Migrations won't run! Please override generateServerId method in migrations configuration!");
    return [];
  }
...
function uncontractedMigrations () {
  ...
  let names = pending.fetch().map(function (m) { return m.name; });
...
function runContract (name) {
  if (Migrations.isExpanded(name)) { runPhase('contract', name); } else { log('contract', name, 'preempted: expand incomplete'); }
}
